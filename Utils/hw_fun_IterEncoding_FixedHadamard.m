function [ang_locs_tagmode,final_encoding_matrix] = hw_fun_IterEncoding_FixedHadamard(VesLocs3D,AngTowardsCor,AngTowardsSag,off_res,number_of_iterations,modmat_phase,modmat_lookup,opts)

% rng(100);
rng(100,'twister'); % 20240730 for HMMD Sub2 Sess1

if_use_simLabEff_modulation = opts.if_use_simLabEff_modulation;
if_fft_based = opts.if_fft_based;
if_display = opts.if_display;
if_3T = opts.if_3T;
usable_Hadamard_order = opts.Hadamard_order;
...
hadamard_matrix = hadamard(usable_Hadamard_order);
...
for i = 1:number_of_iterations
    ...
    if number_of_iterations == 1
        random_order = 2:size(hadamard_matrix, 2);
    else
